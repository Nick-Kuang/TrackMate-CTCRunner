...
public class LacssDetectorModel extends DetectorSweepModel
{

	public LacssDetectorModel()
	{
		super( "Lacss detector", createModels(), createFactory() );
	}

	private static SpotDetectorFactoryBase< ? > createFactory()
	{
		if ( null == new DetectorProvider().getFactory( "LACSS_DETECTOR" ) )
			return null;
		else
			return LacssOpt.createFactory();
	}
	...
	private static Map< String, AbstractParamSweepModel< ? > > createModels()
	{
		if ( null == new DetectorProvider().getFactory( "LACSS_DETECTOR" ) )
		{
			...
			models.put( "", new InfoParamSweepModel()
					.info( "The TrackMate-Lacss module seems to be missing<br>"
							+ "from your Fiji installation. Please follow the link<br>"
							+ "below for installation instructions." )
					.url( "https://imagej.net/plugins/trackmate/trackmate-lacss" ) );
			return models;
		}
		else
		{
			return LacssOpt.createModels();
		}
	}
}
